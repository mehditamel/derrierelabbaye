...
/* Les horaires testés proviennent de data/site.ts :
   Mar–Dim 18:00→02:00, Lun soir fermé.
...
describe("etatOuverture", () => {
  ...
  it("est encore ouvert après minuit (débordement de la veille)", () => {
    // Nuit de mercredi à jeudi, 00h30 : créneau du mercredi ferme à 02h00.
    const etat = etatOuverture(new Date("2026-06-18T00:30:00+02:00"));
    expect(etat.ouvert).toBe(true);
  });

  it("est fermé après l'heure de fermeture", () => {
    // Jeudi 02h30 : le créneau de mercredi (→02h00) est terminé.
    const etat = etatOuverture(new Date("2026-06-18T02:30:00+02:00"));
    expect(etat.ouvert).toBe(false);
  });
});
...
describe("etatOuverture — bords de calendrier", () => {
  it("ferme à 2 h après le service du dimanche", () => {
    // Dimanche 23h30 — le service se poursuit jusqu’à 2 h le lundi.
    const etat = etatOuverture(new Date("2026-06-07T21:30:00Z"));
    expect(etat.ouvert).toBe(true);
    expect(etat.libelle).toContain("02h00");
  });
  ...
  it("est fermé le lundi matin après le débordement du dimanche", () => {
    // Lundi 02h30 : la soirée du dimanche est terminée.
    const etat = etatOuverture(new Date("2026-06-08T00:30:00Z"));
    expect(etat.ouvert).toBe(false);
